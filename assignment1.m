...
imwrite(hp,'HP.png','PNG');
imwrite(lp,'LP.png','PNG');
...
imwrite(fftshift(abs(low_fou))/50,'LP-freq.png','PNG');
imwrite(fftshift(abs(high_fou))/50,'HP-freq.png','PNG');
...
saveas(surf(gaussian_kernel), 'gaus-surf.png');
saveas(surf(dog_kernel), 'dog-surf.png');
...
imwrite(hp_filt,'HP-filt.png','PNG');
imwrite(lp_filt,'LP-filt.png','PNG');
...
% Save the frequency response magnitude as images with adjusted brightness
imwrite(fftshift(abs(hp_filt_freq))/50, 'HP-filt-freq.png','PNG');
imwrite(fftshift(abs(lp_filt_freq))/50, 'LP-filt-freq.png','PNG');
...
imwrite(abs(fftshift(hp_dogfilt_freq)), 'HP-dogfilt-freq.png','PNG');
imwrite(abs(fftshift(lp_dogfilt_freq)), 'LP-dogfilt-freq.png','PNG');
...
imwrite(hp_dogfilt .* 10, 'HP-dogfilt.png', 'PNG');
imwrite(lp_dogfilt .* 10, 'LP-dogfilt.png', 'PNG');
...
imwrite(hp_sub2, 'HP-sub2.png', 'PNG');
imwrite(lp_sub2, 'LP-sub2.png', 'PNG');
...
imwrite(hp_sub2_freq, 'HP-sub2-freq.png', 'PNG');
imwrite(lp_sub2_freq, 'LP-sub2-freq.png', 'PNG');
...
% Save the subsampled images
imwrite(hp_sub4, 'HP-sub4.png', 'PNG');
imwrite(lp_sub4, 'LP-sub4.png', 'PNG');
...
% Save the freq. domain versions of them
imwrite(hp_sub4_freq, 'HP-sub4-freq.png', 'PNG');
imwrite(lp_sub4_freq, 'LP-sub4-freq.png', 'PNG');
...
% Save the subsampled images
imwrite(hp_sub2_aa, 'HP-sub2-aa.png', 'PNG');
imwrite(hp_sub4_aa, 'HP-sub4-aa.png', 'PNG');
...
% Save the freq. domain versions of them
imwrite(hp_sub2_freq_aa, 'HP-sub2-aa-freq.png', 'PNG');
imwrite(hp_sub4_freq_aa, 'HP-sub4-aa-freq.png', 'PNG');

% ** Part 5 **
% Apply Canny edge detection and get the default parameters computed by 
% Matlab to HP
[cannyedge_hp, thresh_hp] = edge(hp, 'canny');
imwrite(edge(hp, 'canny', [0.05, 0.13]), 'HP-canny-optimal.png', 'PNG');
imwrite(edge(hp, 'canny', [0.001, 0.002]), 'HP-canny-lowlow.png', 'PNG');
imwrite(edge(hp, 'canny', [0.09, 0.091]), 'HP-canny-highlow.png', 'PNG');
imwrite(edge(hp, 'canny', [0.001, 0.3]), 'HP-canny-lowhigh.png', 'PNG');
imwrite(edge(hp, 'canny', [0.19, 0.3]), 'HP-canny-highhigh.png', 'PNG');

% Apply Canny edge detection and get the default parameters computed by 
% Matlab to LP
[cannyedge_lp, thresh_lp] = edge(hp, 'canny');
imwrite(edge(lp, 'canny', [0.02, 0.11]), 'LP-canny-optimal.png', 'PNG');
imwrite(edge(lp, 'canny', [0.001, 0.002]), 'LP-canny-lowlow.png', 'PNG');
imwrite(edge(lp, 'canny', [0.08, 0.081]), 'LP-canny-highlow.png', 'PNG');
imwrite(edge(lp, 'canny', [0.001, 0.3]), 'LP-canny-lowhigh.png', 'PNG');
imwrite(edge(lp, 'canny', [0.2, 0.3]), 'LP-canny-highhigh.png', 'PNG');
